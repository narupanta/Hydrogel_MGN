...
class Normalizer(nn.Module):
    ...
    def __init__(self, size, name, max_accumulations=10 ** 6, std_epsilon=1e-8, ):
        ...
        self._acc_count = torch.zeros(1, dtype=torch.float, requires_grad=False).to(device)
        self._num_accumulations = torch.zeros(1, dtype=torch.float, requires_grad=False).to(device)
        self._acc_sum = torch.zeros(size, dtype=torch.float, requires_grad=False).to(device)
        self._acc_sum_squared = torch.zeros(size, dtype=torch.float, requires_grad=False).to(device)
    ...
    def _accumulate(self, batched_data, node_num=None):
        ...
        count = torch.tensor(batched_data.shape[0], dtype=torch.float, device=device)
